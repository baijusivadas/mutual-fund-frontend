...
const Gold = () => {
  ...
  const mutation = useMutation({
    mutationFn: async (payload: any) => {
      if (editingItem) {
        const { error } = await supabase.from('gold').update(payload).eq('id', editingItem.id);
        if (error) throw error;
      } else {
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>Manage your gold inventory and track purchases</CardDescription>
            </div>
            <Dialog open={dialogOpen} onOpenChange={(open) => { setDialogOpen(open); if (!open) resetForm(); }}>
              <DialogTrigger asChild>
                <Button><Plus className="mr-2 h-4 w-4" />Add Gold Item</Button>
              </DialogTrigger>
              <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
                <DialogHeader>
                  <DialogTitle>{editingItem ? "Edit Gold Item" : "Add New Gold Item"}</DialogTitle>
                  <DialogDescription>{editingItem ? "Update gold item details" : "Add a new gold item to your inventory"}</DialogDescription>
                </DialogHeader>
