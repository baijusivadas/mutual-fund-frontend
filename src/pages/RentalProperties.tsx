...
const RentalProperties = () => {
  ...
  const columns: ColumnConfig<RentalProperty>[] = [
    ...
    { key: "monthly_rent", label: "Monthly Rent", sortable: true, render: (item) => `₹${item.monthly_rent.toLocaleString('en-IN')}/mo` },
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>Manage your rental properties and tenant leases</CardDescription>
            </div>
            <Dialog open={dialogOpen} onOpenChange={(open) => { setDialogOpen(open); if (!open) resetForm(); }}>
              <DialogTrigger asChild>
                <Button><Plus className="mr-2 h-4 w-4" />Add Rental Property</Button>
              </DialogTrigger>
              <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
                <DialogHeader>
                  <DialogTitle>{editingProperty ? "Edit Rental Property" : "Add New Rental Property"}</DialogTitle>
                  <DialogDescription>{editingProperty ? "Update rental property details" : "Add a new rental property to your inventory"}</DialogDescription>
                </DialogHeader>
